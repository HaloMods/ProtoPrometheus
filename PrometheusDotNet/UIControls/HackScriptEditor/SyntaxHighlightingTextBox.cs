...
namespace UIControls.HackScriptEditor
{
	...
	public class SyntaxHighlightingTextBox : System.Windows.Forms.RichTextBox 
	{
		...
		{
			get { return mHighlightDescriptors; }
		}
		#endregion
						...
						foreach	(HighlightDescriptor hd in mHighlightDescriptors)
						{
							...
							switch	(hd.DescriptorRecognition)
							{
								case DescriptorRecognition.WholeWord:
									if (curToken == compareStr) match = true;
									break;
								case DescriptorRecognition.StartsWith:
									if (curToken.StartsWith(compareStr)) match = true;
									break;
								case DescriptorRecognition.Contains:
									if (curToken.IndexOf(compareStr) != -1)	match = true;
									break;
							}
							if (!match) // If this token doesn't match chech the next one.
								continue;
		...
		protected override void WndProc(ref Message m)
		{
			switch (m.Msg)
			{
				...
				case Win32.WM_CHAR:
				{
					...
					{
						case Keys.Space:
							if ((Win32.GetKeyState(Win32.VK_CONTROL) & Win32.KS_KEYDOWN )!= 0) return;
							break;
						case Keys.Enter:
