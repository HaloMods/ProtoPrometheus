...
namespace CristiPotlog.Controls
{
    ...
    private WizardPage nextPage;
		...
		#region Properties
    // TODO: Consider creating a designer that allows you to:
    // 1. Select any page from a dropdown from the Wizard in design view.
    // 2. Change/Organize the postiion of pages in the collection.
    // 3. Visually set (and auto set on add) the previous and next pages.
    public WizardPage PreviousPage
	  ...
	  public Wizard Owner
	  {
	    ...
	    set
	    {
	      if (owner != null)
	      {
	        owner.MoveNext -= new Wizard.ChangePageEventHandler(owner_MoveNext);
          owner.PageChanged -= new Wizard.PageChangedEventHandler(owner_PageChanged);
	      }
        owner = value;
        owner.MoveNext += new Wizard.ChangePageEventHandler(owner_MoveNext);
        owner.PageChanged += new Wizard.PageChangedEventHandler(owner_PageChanged);
	  ...
	  private void owner_PageChanged(object newPage)
	  {
      // Make sure that this event concerns the current page.
      if (newPage != this) return;
      this.OnBeforeShow();
	  }

	  private void owner_MoveNext(object sender, Wizard.ChangePageEventArgs e)
	  {
	    // Make sure that this event concerns the current page.
      if (e.CurrentPage != this) return;
      this.OnBeforeNext(e);
	  }
    ...
    public event Wizard.ChangePageEventHandler BeforeNext;
    protected void OnBeforeNext(Wizard.ChangePageEventArgs e)
    {
      e.NextPage = this.nextPage;
      if (BeforeNext != null) BeforeNext(this, e);
      if (e.Cancel) return;
    }
    #endregion
    ...
    /// <summary>
	  ...
	  {
	    if (BeforeShow != null) BeforeShow(this, new EventArgs());
	  }
