...
namespace CristiPotlog.Controls
{
	...
	public class Wizard : XtraUserControl
	{
    ...
    private DevExpress.XtraEditors.SimpleButton buttonNext;
      ...
      this.ResumeLayout(false);
		...
		public void Next()
		{
			...
			{
				// We will assume the very next page for now.
				this.OnChangePage(new ChangePageEventArgs(this.Pages[this.SelectedIndex], null));
			}
		}
		...
		public void Back()
		{
			...
			{
				// handle page switch
				// I don't think that we really need to worry with an event for moving back.
        // If we do though, it needs to be handled differently than moving forward and needs
        // to be routed through the page object itself.
        //this.OnBeforeSwitchPages(new BeforeSwitchPagesEventArgs(this.SelectedIndex, this.SelectedIndex - 1));
        this.ActivatePage(this.pages[this.SelectedIndex].PreviousPage);
      }
		}
      ...
      if (PageChanged != null) PageChanged(page);
      ...
      if (e.NextPage == null) e.NextPage = this.Pages[this.SelectedIndex+1];
...
//		public event AfterSwitchPagesEventHandler AfterSwitchPages;
    ...
    public delegate void PageChangedEventHandler(object newPage);

    public event ChangePageEventHandler MoveNext;
    ...
    {
      if (MoveNext != null) MoveNext(this, e);
      if (e.Cancel) return;
    }
    /// <summary>
    /// Provides data for the BeforeNextEventArgs event of the WizardPage control.
    /// </summary>
    public class ChangePageEventArgs : CancelEventArgs
    {
      private WizardPage nextPage;
      private WizardPage currentPage;

      public ChangePageEventArgs(WizardPage currentPage, WizardPage nextPage)
      {
