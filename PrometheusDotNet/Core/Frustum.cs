...
namespace SharpEdit
{
	...
	{
		private int A = 0;				// The X value of the plane's normal
		private int B = 1;				// The Y value of the plane's normal
		private int C = 2;				// The Z value of the plane's normal
		private int D = 3;				// The distance the plane is from the origin
		...
		private float[][] m_Frustum = new float[6][] {
			new float[4], new float[4], new float[4], new float[4], new float[4], new float[4] };
		...
		public void NormalizePlane(float[][] frustum, int side) // [6][4]
		{
		...
		public bool BoxInFrustum(CBoundingBox Box)
		{
			float[] min = Box.min;
			float[] max = Box.max;
			...
			for (int i = 0; i < 6; i++)
			{
				if(m_Frustum[i][A] < 0.0f)
				{
					if(m_Frustum[i][B] < 0.0f)
					{
						if(m_Frustum[i][C] < 0.0f)
						{
							NearPoint[0] = min[0];
							NearPoint[1] = min[1]; 
							NearPoint[2] = min[2];
				...
				if(Util.DotProduct(m_Frustum [i], NearPoint) + m_Frustum[i][D] < 0)
				return false;
			}
			return true;
		}
